...
export const Engine = () => {
  ...
  const [passwords, setPasswords] = useState<
    Array<{ website: string; username: string; password: string }>
  >([]);
  ...
  const fetchPasswords = useCallback(async () => {
    try {
      ...
      if (userApiKey != null) {
        const getPasswordsResponse = await fetch(
          `https://api.tacotools.dev/api/v1/taco-passwords/get-passwords`,
          {
            method: "GET",
            mode: "cors",
            headers: {
        ...
        if (getPasswordsResponse.status !== 404) {
          const getPasswordsResult: GetPasswordsResponseDTO =
            await getPasswordsResponse.json();
    ...
    try {
      ...
      if (userApiKey != null) {
        const response = await fetch(
          "https://api.tacotools.dev/api/v1/taco-passwords/add-password",
          {
            method: "POST",
            mode: "cors",
            headers: {
  ...
  const updatePasswordToDB = async (index: number) => {
    ...
    try {
      ...
      if (userApiKey != null) {
        const response = await fetch(
          "https://api.tacotools.dev/api/v1/taco-passwords/update-password",
          {
            method: "PUT",
            mode: "cors",
            headers: {
        ...
        if (response.ok) {
          ...
          const errorData = await response.json();
          setError(
            new Error(errorData.message || "Failed to update password."),
          );
        }
      }
    } catch (error: any) {
  ...
  const deletePassword = async (index: number) => {
    ...
    try {
      ...
      if (userApiKey != null) {
        const response = await fetch(
          "https://api.tacotools.dev/api/v1/taco-passwords/delete-password",
          {
            method: "DELETE",
            mode: "cors",
            headers: {
        ...
        if (response.ok) {
          ...
          const errorData = await response.json();
          setError(
            new Error(errorData.message || "Failed to delete password."),
          );
        }
      }
    } catch (error: any) {
