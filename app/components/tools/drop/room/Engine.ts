...
export const Engine = () => {
  ...
  const deleteRoom = async () => {
    ...
    try {
      ...
      const deleteRoomResult = await fetch(
        `https://api.tacotools.dev/api/v1/taco-drop/delete-rooms`,
        {
          method: "DELETE",
          mode: "cors",
          headers: {
      ...
      try {
        ...
        if (userApiKey != null) {
          const getFileResponse = await fetch(
            `https://api.tacotools.dev/api/v1/taco-drop/get-file?roomID=${roomId}`,
            {
              method: "GET",
              mode: "cors",
              headers: {
          ...
          if (getFileResponse.status !== 404) {
            const getFileResult: GetFileResponseDTO =
